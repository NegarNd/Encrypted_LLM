...
def vmm_q_gqa(
    ...
    Q_cts = np.zeros((dims.ratio, dims.N), dtype=np.float64)
...
def qkt_gqa(Q_cts: np.ndarray, k_ciphertexts: List[np.ndarray], dims: GQADims) -> np.ndarray:
    ...
    att_cts = np.zeros((dims.ratio, len(k_ciphertexts), dims.N), dtype=np.float64)
...
def softmax_v_gqa(att_cts: np.ndarray, vcache: VCache, dims: GQADims) -> np.ndarray:
    ...
    out_mask = np.zeros(dims.N, dtype=np.float64)
    out_mask[:: dims.t_p] = 1.0
    O_ct = np.zeros((dims.ratio, dims.N), dtype=np.float64)
...
def run_attention_gqa(
    ...
    dims = make_gqa_dims(GQAConfig(N=N, d=d, H=H, n_kv=n_kv, n_prefill=n_prefill))
    ...
    kcache = KCache(dims.N, dims.d_kv)
    vcache = VCache(dims.N, dims.d_kv, H=dims.n_kv)

    toks = [init_input(dims.d, seed=10 + i) for i in range(n_prefill)]
    for x in toks:
        xc = preprocess_input_kv(x, dims)
    ...
    xc_new = preprocess_input_kv(x_new, dims)
    ...
    if verbose:
        ...
        print(
            f"N={dims.N:3d} d={dims.d:2d} H={dims.H} n_kv={dims.n_kv} "
